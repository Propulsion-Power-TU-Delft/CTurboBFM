...
def save_csv_boundaries(X, Y, iblock):
    ...
    i_start_bump = np.argmin(np.abs(X[iblock][:,0] - 0.0)) # this is the primary node of the bump start
    i_end_bump = np.argmin(np.abs(X[iblock][:,0] - 1.5)) # this is the secondary node of the bump start
    ...
    patches["SYMMETRIC_INLET"] = {"I_MIN": 0, "I_MAX": i_start_bump+1, "J_MIN": 0, "J_MAX": 0, "K_MIN": 0, "K_MAX": 1}
    patches["BUMP"] = {"I_MIN": i_start_bump+1, "I_MAX": i_end_bump+1, "J_MIN": 0, "J_MAX": 0, "K_MIN": 0, "K_MAX": 1}
    patches["SYMMETRIC_OUTLET"] = {"I_MIN": i_end_bump+1, "I_MAX": ni, "J_MIN": 0, "J_MAX": 0, "K_MIN": 0, "K_MAX": 1}
